...
class VideoDetailFrame(Frame):
    ...
    def create_widgets(self):
        ...
        # Header
        self.label = customtkinter.CTkLabel(self, text=self.root.video.yt.title, font=self.root.font_header, wraplength=600)
        self.label.grid(row=0, column=0, columnspan=2, padx=10, pady=(20,40))
        
        # Left column - Image
        self.thumbnail = customtkinter.CTkImage(light_image=Image.open(self.root.video.thumbnail), size=(195, 110))
        self.thumbnail_label = customtkinter.CTkLabel(self, image=self.thumbnail, text="")
        self.thumbnail_label.grid(row=1, column=0, rowspan=3, padx=10, pady=5, stick="we")
        
        # Right column - Video details
        self.author = customtkinter.CTkLabel(self, text="Author: " + self.root.video.yt.author, font=self.root.font, wraplength=200)
        ...
        self.button = customtkinter.CTkButton(self, width=self.root.button_width, height=self.root.button_height, text="Download options", font=self.root.font, command=self.go_to_download_options)
...
class DownloadOptionsFrame(Frame):
    ...
    def create_widgets(self):
        ...
        # Stream selection
        self.label1 = customtkinter.CTkLabel(self, text="Please select which stream to download:", font=self.root.font_header)
        ...
        self.combo_streams = customtkinter.CTkComboBox(self, height=self.root.combobox_height, values=list(self.available_streams.keys()), variable=self.selected_stream)
        self.combo_streams.grid(row=1, column=0, columnspan=2)
        self.combo_streams.set(next(iter(self.available_streams.keys())))   
        
        # Download path selection
        self.label2 = customtkinter.CTkLabel(self, text="Please select where to save the file:", font=self.root.font_header)
        ...
        self.button_browse = customtkinter.CTkButton(self, width=self.root.button_width, height=self.root.button_height, text="Browse", font=self.root.font, command=self.process_path_browse)
        self.button_browse.grid(row=4, column=0, columnspan=2, padx=10, pady=10, sticky="ne")

        # Navigation buttons
        self.button_back = customtkinter.CTkButton(self, width=self.root.button_width, height=self.root.button_height, text="Back", font=self.root.font, command=self.back_to_video_details)
        self.button_back.grid(row=5, column=0, padx=10, pady=(40,10), sticky="e")
        self.button = customtkinter.CTkButton(self, width=self.root.button_width, height=self.root.button_height, text="Download", font=self.root.font, command=self.start_download)
    ...
    def get_streams(self):      
        ...
        for streams in [streams_video, streams_audio]:
            for stream in streams:
                desc = f"Video {stream.abr}" if stream in streams_video else f"Audio {stream.abr}"
    ...
    def start_download(self):
        ...
        self.root.stream.stream_id = self.available_streams.get(self.combo_streams.get())
