...
class Stream:
    ...
    def set_download_filename(self, original_title: str, max_length: int = 50) -> str:
        ...
        self.download_filename = sanitized_title[:max_length].lower().rstrip("_")

    def set_download_full_filename(
        self, prefix: str, filename: str, extension: str
    ) -> None:
    ...
    def download_video_best_quality(self) -> None:
        self.stream_id = (
            self.video.yt.streams.filter(progressive=True)
            .order_by("resolution")
            .desc()
            .first()
            .itag
    ...
    def download(
        ...
        download_filename_extension: str = ".mp4",
    ) -> None:
